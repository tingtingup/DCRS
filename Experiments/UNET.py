...
class unet3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # up_6 = self.up6(c5)
        up_5 = self.up7(c4)
        # up_5 = up_5[:, :, 0:-1, 0:-1, 0:-1]          #############gai
        up_5 = up_5[:, :, :, 0:-1, 0:-1]          ## resample
        # c3 = c3[:, :, :, 0:-1, :]                   ########gai
        # c3 = c3[:, :, :, :, :]
        # print(up_5.shape)
        # print(c3.shape)
        merge6 = torch.cat([up_5, c3], dim=1)
        c6 = self.conv7(merge6)

        up_7 = self.up8(c6)
        # up_7 = F.pad(up_7, [0, 0, 1, 1])
        # up_7 = F.pad(up_7, [0, 0, 1, 1, 0 ,0 ])   ########gai
        up_7 = up_7[:, :, 0:-1, :, 0:-1]     ##resample
        # print(up_7.shape)
        # print(c2.shape)
        # up_7 = up_7[:, :, 0:-1, 1:-1, 0:-1]

        merge7 = torch.cat([up_7, c2], dim=1)
        c7 = self.conv8(merge7)
